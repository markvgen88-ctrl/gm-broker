...
> Если канал уже был настроен раньше (до появления поля «Телефон» в форме):
> добавьте столбец «Телефон» в таблицу между «Имя» и «E-mail» и обновите код
> Apps Script (Шаг 2) — строка `data.phone || "",` должна идти сразу после
> `data.name || "",`. Иначе телефон просто не попадёт в резервную таблицу.
...
3. В первой строке (заголовки) впишите:
   ...
   ```
   Дата отправки | Тип клиента | Имя | Телефон | E-mail | Сумма | Цель | Полная анкета (JSON)
   ```
