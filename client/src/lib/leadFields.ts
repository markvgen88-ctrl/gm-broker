...
/**
 * Подписи и форматирование полей анкеты для отображения в /admin.
 * Зеркалит server/src/lib/fields.ts — при изменении структуры анкеты
 * на фронтенде (client/src/data/questionnaire.ts) обновите оба файла.
 */
...
const YES_NO_VALUES: Record<string, string> = { yes: "Да", no: "Нет" };
...
export function formatFieldValue(field: string, value: unknown): string {
  ...
  if (MONEY_FIELDS.has(field)) return formatCurrency(value as number | string);
