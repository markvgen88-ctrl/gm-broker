 ...
 *    отсутствии подходящего залога анкета заканчивается отказом.
 *    Если просрочек не было вовсе — сценарий идёт по "чистой" ветке
 *    (суффикс _1) с более коротким набором вопросов по недвижимости.
 *    Отсутствие дохода (ни официального, ни неофициального) само по себе
 *    больше не ведёт к отказу — анкета продолжается вопросом(-ами) про
 *    действующие кредиты и переходит в общую с "усиленной" веткой цепочку
 *    вопросов про недвижимость/авто (узел f_property_2 и далее), которая
 *    и решает, отказ это или нет.
...
export const questionnaireGraph: QuestionnaireGraph = {
  ...
  // трудоустроены официально нет — доход только неофициальный. Раньше при
  // его отсутствии анкета сразу заканчивалась отказом; теперь вместо этого
  // уточняем действующие кредиты и уходим в общую цепочку вопросов про
  // недвижимость/авто (см. f_active_loans_no_income) — доход сам по себе
  // отказ больше не определяет.
  f_unofficial_q_1a: {
    ...
    options: [
      ...
      { label: "Нет", value: "no", next: "f_active_loans_no_income" },
    ],
  },
  ...
  // дохода нет вовсе (ни официального, ни неофициального) — вместо отказа
  // уточняем действующие кредиты и дальше уходим в общую с "усиленной"
  // веткой цепочку узлов (f_loan_balance_2 → … → f_property_2 → авто),
  // которая уже сама решает, достаточно ли у клиента подходящего залога.
  f_active_loans_no_income: {
    id: "f_active_loans_no_income",
    kind: "choice",
    field: "hasActiveLoans",
    question: "Есть ли у вас действующие кредиты?",
    options: [
      { label: "Да", value: "yes", next: "f_loan_balance_2" },
      { label: "Нет", value: "no", next: "f_property_2" },
    ],
  },
  ...
  // дохода нет вовсе — раньше это сразу вело к отказу. Теперь, поскольку в
  // этой ветке действующий кредит и так предполагается (просрочка уже
  // есть/была), вопросы про остаток/платежи/МФО/крупнейший кредит
  // пропускаются, и клиент сразу переходит к вопросу про недвижимость
  // (f_property_2) — отказ, если он будет, определится уже там, по
  // наличию подходящего залога.
  f_unofficial_q_2a: {
  ...
  // ---- отказ по ветке «Физлицо» ----
  // Единственный оставшийся тупик анкеты для физлиц: нет ни своей, ни
  // родственной недвижимости под залог (или она не подходит), и нет
  // автомобиля — то есть заведомо нет подходящего залога для кредита.
